...
export default function AlbumPage() {
  ...
  const handleStatusChange = useCallback(async (stickerId: number, status: StickerStatus) => {
    setUpdating(stickerId);
    try {
      await api.put(`/api/album/stickers/${stickerId}`, { status });
      setStickers((prev) => prev.map((s) =>
        s.stickerId === stickerId
          ? { ...s, status, repeatedCount: status === 'NAO_TENHO' ? 0 : s.repeatedCount }
          : s
      ));
  ...
  const faltam = stickers.filter((s) => s.status === 'NAO_TENHO').length;
  const pct    = stickers.length > 0 ? Math.round(tenho / stickers.length * 1000) / 10 : 0;

  const filtered = stickers.filter((s) => {
    const flt = filter === 'TODOS' || s.status === filter;
    const src = search === '' ||
      ...
      <div className="min-h-screen bg-gray-100">
        ...
        <main className="max-w-7xl mx-auto px-4 py-6">
          <div className="rounded-2xl p-5 mb-5 shadow-lg"
            ...
            <div className="w-full h-2.5 bg-white/10 rounded-full overflow-hidden mb-4">
              <div className="h-full rounded-full transition-all duration-700"
                   style={{ width: `${pct}%`, background: 'linear-gradient(90deg, #C4A135, #e8bf4a)' }} />
            </div>
          ...
          <div className="flex items-center gap-5 mb-4 px-1 text-xs text-gray-500">
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded border-2 border-red-400 bg-red-50 inline-block"/>Não tenho</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded border-2 border-green-500 bg-green-50 inline-block"/>Tenho</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded border-2 inline-block" style={{ borderColor:'#C4A135', backgroundColor:'rgba(196,161,53,0.1)' }}/>Com repetidas</span>
          </div>
                ...
                {([
                  ...
                  <button key={key} onClick={() => setFilter(key as Filter)}
                    className={`px-3 py-2 rounded-xl text-xs font-bold transition-all border
                      ${filter === key ? `${active} border-transparent` : 'bg-gray-100 text-gray-600 border-gray-200 hover:bg-gray-200'}`}>
                    {label}
                  </button>
                ))}
              </div>
          ...
          {loading ? (
            ...
            Object.entries(grouped)
              ...
              .map(([team, items]) => {
                ...
                const tTenho = items.filter((s) => s.status === 'TENHO').length;
                const tFalt  = items.filter((s) => s.status === 'NAO_TENHO').length;
                return (
                  <div key={team} className="mb-4">
                    <button onClick={() => setExpanded((p) => ({ ...p, [team]: !p[team] }))}
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {tTenho > 0 && <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-bold">✓ {tTenho}</span>}
                        {tFalt  > 0 && <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full font-bold">✗ {tFalt}</span>}
                      </div>
                    ...
                    {open && (
                      <div className="grid gap-1.5"
                        ...
                        {items.map((s) => (
                          <StickerCard key={s.stickerId} sticker={s}
                            onStatusChange={handleStatusChange}
                            loading={updating === s.stickerId} />
                        ))}
                      </div>
                    )}
                  </div>
