      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              { label: 'Nome', value: name, set: setName, type: 'text', ph: 'Seu nome', Icon: User },
              { label: 'Email', value: email, set: setEmail, type: 'email', ph: 'seu@email.com', Icon: Mail },
              { label: 'Senha', value: password, set: setPassword, type: 'password', ph: 'Mínimo 6 caracteres', Icon: Lock },
            ].map(({ label, value, set, type, ph, Icon }) => (
              <div key={label}>
                <label className="block text-sm font-medium text-white/70 mb-1.5">{label}</label>
                <div className="relative">
                  <Icon className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30" size={16} />
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl
                               ...
                               focus:outline-none focus:border-galo-gold focus:ring-1 focus:ring-galo-gold transition-all"
                  />
                </div>
              </div>
            ))}
